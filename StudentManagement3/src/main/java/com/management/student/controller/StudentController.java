...
public class StudentController {
     ...
     */
   @RequestMapping(value = RequestURL.STUDENTS_PAGINATION, method = RequestMethod.GET)
   @ResponseBody
    public String studentPagination(final HttpServletRequest request)
        throws IOException {

        String textToSearch = request.getParameter("sSearch");
        String columnToSort = request.getParameter("iSortCol_0");
        ...
        PaginationDto paginationDto = new PaginationDto(textToSearch, columnToSort, sortingDir, displayStart,
            displayLength);

        List<Student> studentList;
        if (paginationDto.isFirstIteration()) {
            studentList = this.studentService.getAll();
        } else {
            studentList = this.studentService.getAll(paginationDto);
        }
   ...
   @RequestMapping(value = RequestURL.STUDENTS_LIST, method = RequestMethod.GET)
    public ModelAndView getAll(final HttpServletRequest request)
        ...
        studentForm.setMessage(this.messageSource.getMessage(MessageEnum.VALID_CRIDENTIAL.getValue(), new Object[] {},
    ...
    public ModelAndView save(@PathVariable(FormParams.ROLL_NO) final String rollNo,
        ...
        this.studentModifyValidator.validate(studentForm, result);
        ...
        if (!result.hasErrors()) {
            ...
            studentForm.setStudentList(studentList);
            studentForm.setStatus(Constant.SUCCESS);
            studentForm.setMessage(this.messageSource.getMessage(MessageEnum.STUDENT_UPDATE_SUCCESS.getValue(), new Object[]{rollNo}, LocaleContextHolder.getLocale()));
            outParam.addObject("StudentForm", studentForm);
    ...
    public ModelAndView removeStudent(@PathVariable(FormParams.ROLL_NO) final String rollNo,
        ...
        studentFrom.setMessage(this.messageSource.getMessage(MessageEnum.STUDENT_DELETE_SUCCESS.getValue(), new Object[]{rollNo}, LocaleContextHolder.getLocale()));
    ...
    public ModelAndView add(@ModelAttribute("StudentForm") final StudentForm studentForm ,final BindingResult result,
        ...
        this.studentAddValidator.validate(studentForm, result);
        if (!result.hasErrors()) {
            ...
            if (!this.studentService.alreadyExist(rollNumber)) {
                ...
                studentForm.setMessage(this.messageSource.getMessage(MessageEnum.STUDENT_ADD_SUCCESS.getValue(), new Object[]{studentForm
                ...
                studentForm.setStudentList(studentList);
                studentForm.setStatus(Constant.ERROR);
                studentForm.setMessage(this.messageSource.getMessage(MessageEnum.STUDENT_ALREADY_EXIST.getValue(), new Object[]{studentForm.getAddStudent().getRollNumber()}, LocaleContextHolder.getLocale()));
                outParam.addObject("StudentForm", studentForm);
            }
        } else {
            List<Student> studentList = this.studentService.getAll();
